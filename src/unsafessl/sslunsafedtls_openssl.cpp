...
QByteArray cookie_for_peer(SSL *ssl)
{
    ...
    auto listener = static_cast<dtlsopenssl::SslUnsafeDtlsState *>(q_BIO_get_app_data(readBIO));
    if (!listener) {
...
// by SslUnsafeDtls, only a handshake (which is allowed to fragment).
                                          ...
                                          unsigned *cookieLength)
{
    ...
    void *generic = q_SSL_get_ex_data(ssl, SslUnsafeSocketBackendPrivate::s_indexForSSLExtraData);
    if (!generic) {
    ...
    *cookieLength = 0;
    ...
    auto dtls = static_cast<dtlsopenssl::SslUnsafeDtlsState *>(generic);
    if (!dtls->secret.size())
        return 0;
...
extern "C" int q_X509DtlsCallback(int ok, X509_STORE_CTX *ctx)
{
    if (!ok) {
        ...
        auto dtls = static_cast<dtlsopenssl::SslUnsafeDtlsState *>(generic);
        dtls->x509Errors.append(SslUnsafeErrorEntry::fromStoreContext(ctx));
    }
...
{
    auto *dtls = static_cast<dtlsopenssl::SslUnsafeDtlsState *>(q_SSL_get_ex_data(ssl,
                            SslUnsafeSocketBackendPrivate::s_indexForSSLExtraData));
    if (!dtls)
        return 0;
...
extern "C" int q_dgram_read(BIO *bio, char *dst, int bytesToRead)
{
    ...
    auto dtls = static_cast<dtlsopenssl::SslUnsafeDtlsState *>(q_BIO_get_app_data(bio));
...
extern "C" int q_dgram_write(BIO *bio, const char *src, int bytesToWrite)
{
    ...
    if (dtls->writeSuppressed) {
        // See the comment in SslUnsafeDtls::startHandshake.
        return bytesToWrite;
    }
...
#if OPENSSLV10 // #if !QT_CONFIG(opensslv11)
    Q_UNUSED(num)
#endif
...
bool SslUnsafeDtlsState::init(SslUnsafeDtlsBasePrivate *dtlsBase, QUdpSocket *socket,
...
void SslUnsafeDtlsState::reset()
{
    tlsConnection.reset();
    tlsContext.reset();
}

bool SslUnsafeDtlsState::initTls(SslUnsafeDtlsBasePrivate *dtlsBase)
{
    if (tlsContext.data())
        return true;
...
{
    //: %1: Some function
    return SslUnsafeDtls::tr("%1 failed").arg(QLatin1String(function));
}

bool SslUnsafeDtlsState::initCtxAndConnection(SslUnsafeDtlsBasePrivate *dtlsBase)
{
    Q_ASSERT(dtlsBase);
    Q_ASSERT(SslUnsafeSocket::supportsSsl());

    if (dtlsBase->mode == SslUnsafeSocket::UnencryptedMode) {
        dtlsBase->setDtlsError(SslUnsafeDtlsError::TlsInitializationError,
                               SslUnsafeDtls::tr("Invalid SslMode, SslServerMode or SslClientMode expected"));
        return false;
    }

    if (!SslUnsafeDtlsBasePrivate::isDtlsProtocol(dtlsBase->dtlsConfiguration.protocol)) {
        dtlsBase->setDtlsError(SslUnsafeDtlsError::TlsInitializationError,
                               SslUnsafeDtls::tr("Invalid protocol version, DTLS protocol expected"));
        return false;
    }

    // Create a deep copy of our configuration
    auto configurationCopy = new SslUnsafeConfigurationPrivate(dtlsBase->dtlsConfiguration);
    ...
    TlsContext newContext(SslUnsafeContext::sharedFromConfiguration(dtlsBase->mode,
                                                               configurationCopy,
                                                               dtlsBase->dtlsConfiguration.allowRootCertOnDemandLoading));

    if (newContext->error() != SslUnsafeError::NoError) {
        dtlsBase->setDtlsError(SslUnsafeDtlsError::TlsInitializationError, newContext->errorString());
        return false;
    }
    ...
    if (set != 1 && configurationCopy->peerVerifyMode != SslUnsafeSocket::VerifyNone) {
        dtlsBase->setDtlsError(SslUnsafeDtlsError::TlsInitializationError,
                               msgFunctionFailed("SSL_set_ex_data"));
        return false;
    }

    if (dtlsBase->mode == SslUnsafeSocket::SslServerMode) {
...
bool SslUnsafeDtlsState::initBIO(SslUnsafeDtlsBasePrivate *dtlsBase)
{
    Q_ASSERT(dtlsBase);
...
void SslUnsafeDtlsState::setLinkMtu(SslUnsafeDtlsBasePrivate *dtlsBase)
{
    Q_ASSERT(dtlsBase);
    Q_ASSERT(udpSocket);
...
SslUnsafeDtlsClientVerifierOpenSSL::SslUnsafeDtlsClientVerifierOpenSSL()
{
    secret = dtlsutil::fallbackSecret();
}

bool SslUnsafeDtlsClientVerifierOpenSSL::verifyClient(QUdpSocket *socket, const QByteArray &dgram,
                                              const QHostAddress &address, quint16 port)
{
    ...
    if (!peer.data()) {
        setDtlsError(SslUnsafeDtlsError::TlsInitializationError,
                     SslUnsafeDtlsClientVerifier::tr("BIO_ADDR_new failed, ignoring client hello"));
        return false;
    }
    ...
    if (ret < 0) {
        ...
        setDtlsError(SslUnsafeDtlsError::TlsFatalError, SslUnsafeSocketBackendPrivate::getErrorsFromOpenSsl());
        return false;
    }
#else
    qt_sockaddr peer;
...
void SslUnsafeDtlsPrivateOpenSSL::TimeoutHandler::start(int hintMs)
{
    Q_ASSERT(timerId == -1);
    timerId = startTimer(hintMs > 0 ? hintMs : timeoutMs, Qt::PreciseTimer);
}

void SslUnsafeDtlsPrivateOpenSSL::TimeoutHandler::doubleTimeout()
{
...
void SslUnsafeDtlsPrivateOpenSSL::TimeoutHandler::stop()
{
    if (timerId != -1) {
        killTimer(timerId);
        timerId = -1;
...
void SslUnsafeDtlsPrivateOpenSSL::TimeoutHandler::timerEvent(QTimerEvent *event)
{
    Q_UNUSED(event)
    Q_ASSERT(timerId != -1);
...
SslUnsafeDtlsPrivateOpenSSL::SslUnsafeDtlsPrivateOpenSSL()
{
    secret = dtlsutil::fallbackSecret();
    dtls.dtlsPrivate = this;
}

bool SslUnsafeDtlsPrivateOpenSSL::startHandshake(QUdpSocket *socket, const QByteArray &dgram)
{
    Q_ASSERT(socket);
    Q_ASSERT(handshakeState == SslUnsafeDtls::HandshakeNotStarted);
    ...
    if (mode == SslUnsafeSocket::SslServerMode && dtlsConfiguration.dtlsCookieEnabled) {
        dtls.secret = secret;
        dtls.hashAlgorithm = hashAlgorithm;
        ...
        if (!peer.data()) {
            setDtlsError(SslUnsafeDtlsError::TlsInitializationError,
                         SslUnsafeDtls::tr("BIO_ADD_new failed, cannot start handshake"));
            return false;
        }

        // If it's an invalid/unexpected ClientHello, we don't want to send
        // VerifyClientRequest - it's a job of SslUnsafeDtlsClientVerifier - so we
...
#endif
        if (result <= 0) {
            setDtlsError(SslUnsafeDtlsError::TlsFatalError,
                         SslUnsafeDtls::tr("Cannot start the handshake, verified client hello expected"));
            dtls.reset();
            return false;
        }
    }

    handshakeState = SslUnsafeDtls::HandshakeInProgress;
    opensslErrors.clear();
    tlsErrors.clear();
...
bool SslUnsafeDtlsPrivateOpenSSL::continueHandshake(QUdpSocket *socket, const QByteArray &dgram)
{
    Q_ASSERT(socket);

    Q_ASSERT(handshakeState == SslUnsafeDtls::HandshakeInProgress);
    ...
    int result = 0;
    if (mode == SslUnsafeSocket::SslServerMode)
        result = q_SSL_accept(dtls.tlsConnection.data());
    else
        result = q_SSL_connect(dtls.tlsConnection.data());

    // DTLSTODO: Investigate/test if it makes sense - SslUnsafeSocket can emit
    ...
    if (result <= 0) {
        ...
        default:
            ...
            setDtlsError(SslUnsafeDtlsError::TlsFatalError,
                         SslUnsafeSocketBackendPrivate::msgErrorsDuringHandshake());
            dtls.reset();
            handshakeState = SslUnsafeDtls::HandshakeNotStarted;
            return false;
        }
    }
    ...
    const bool doVerifyPeer = dtlsConfiguration.peerVerifyMode == SslUnsafeSocket::VerifyPeer
                              || (dtlsConfiguration.peerVerifyMode == SslUnsafeSocket::AutoVerifyPeer
                                  && mode == SslUnsafeSocket::SslClientMode);

    if (!doVerifyPeer || verifyPeer() || tlsErrorsWereIgnored()) {
        connectionEncrypted = true;
        handshakeState = SslUnsafeDtls::HandshakeComplete;
        return true;
    }

    setDtlsError(SslUnsafeDtlsError::PeerVerificationError, SslUnsafeDtls::tr("Peer verification failed"));
    handshakeState = SslUnsafeDtls::PeerVerificationFailed;
    return false;
}


bool SslUnsafeDtlsPrivateOpenSSL::handleTimeout(QUdpSocket *socket)
{
    Q_ASSERT(socket);
...
bool SslUnsafeDtlsPrivateOpenSSL::resumeHandshake(QUdpSocket *socket)
{
    Q_UNUSED(socket);
    Q_ASSERT(socket);
    Q_ASSERT(handshakeState == SslUnsafeDtls::PeerVerificationFailed);
...
void SslUnsafeDtlsPrivateOpenSSL::abortHandshake(QUdpSocket *socket)
{
    Q_ASSERT(socket);
    Q_ASSERT(handshakeState == SslUnsafeDtls::PeerVerificationFailed
             || handshakeState == SslUnsafeDtls::HandshakeInProgress);
    ...
    if (handshakeState == SslUnsafeDtls::PeerVerificationFailed) {
...
void SslUnsafeDtlsPrivateOpenSSL::sendShutdownAlert(QUdpSocket *socket)
{
    Q_ASSERT(socket);
...
qint64 SslUnsafeDtlsPrivateOpenSSL::writeDatagramEncrypted(QUdpSocket *socket,
                                                   const QByteArray &dgram)
{
    ...
    case SSL_ERROR_ZERO_RETURN:
        ...
        setDtlsError(SslUnsafeDtlsError::TlsFatalError, SslUnsafeDtls::tr("The DTLS connection has been closed"));
        handshakeState = SslUnsafeDtls::HandshakeNotStarted;
        dtls.reset();
        break;
    ...
    default:
        ...
        // Unlike SslUnsafeSocket we do not abort though.
        QString description(SslUnsafeSocketBackendPrivate::getErrorsFromOpenSsl());
        if (socket->error() != QAbstractSocket::UnknownSocketError && description.isEmpty()) {
            setDtlsError(SslUnsafeDtlsError::UnderlyingSocketError, socket->errorString());
        } else {
            setDtlsError(SslUnsafeDtlsError::TlsFatalError,
                         SslUnsafeDtls::tr("Error while writing: %1").arg(description));
        }
    }
...
QByteArray SslUnsafeDtlsPrivateOpenSSL::decryptDatagram(QUdpSocket *socket, const QByteArray &tlsdgram)
{
    Q_ASSERT(socket);
    ...
    case SSL_ERROR_ZERO_RETURN:
        ...
        connectionWasShutdown = true;
        setDtlsError(SslUnsafeDtlsError::RemoteClosedConnectionError,
                     SslUnsafeDtls::tr("The DTLS connection has been shutdown"));
        dtls.reset();
        connectionEncrypted = false;
        handshakeState = SslUnsafeDtls::HandshakeNotStarted;
        return dgram;
    ...
    default:
        setDtlsError(SslUnsafeDtlsError::TlsNonFatalError,
                     SslUnsafeDtls::tr("Error while reading: %1")
                               .arg(SslUnsafeSocketBackendPrivate::getErrorsFromOpenSsl()));
        return dgram;
    }
}

unsigned SslUnsafeDtlsPrivateOpenSSL::pskClientCallback(const char *hint, char *identity,
                                                unsigned max_identity_len,
                                                unsigned char *psk,
                                                unsigned max_psk_len)
{
    // The code below is taken (with some modifications) from qsslsocket_openssl
    // - alas, we cannot simply re-use it, it's in SslUnsafeSocketPrivate.
...

    {
        ...
        if (hint) {
            ...
            // From the original code in SslUnsafeSocket:
...
unsigned SslUnsafeDtlsPrivateOpenSSL::pskServerCallback(const char *identity, unsigned char *psk,
                                                unsigned max_psk_len)
{
    Q_Q(SslUnsafeDtls);
...
SslUnsafeError _q_OpenSSL_to_SslUnsafeError(int errorCode, const SslUnsafeCertificate &cert);

bool SslUnsafeDtlsPrivateOpenSSL::verifyPeer()
{
    // DTLSTODO: Windows-specific code for CA fetcher is not here yet.
    QVector<SslUnsafeError> errors;

    // Check the whole chain for blacklisting (including root, as we check for
    // subjectInfo and issuer)
    for (const SslUnsafeCertificate &cert : const_cast<QList<SslUnsafeCertificate> &>(dtlsConfiguration.peerCertificateChain)) { // qAsConst(dtlsConfiguration.peerCertificateChain)
        if (SslUnsafeCertificatePrivate::isBlacklisted(cert))
            errors << SslUnsafeError(SslUnsafeError::CertificateBlacklisted, cert);
    }

    if (dtlsConfiguration.peerCertificate.isNull()) {
        errors << SslUnsafeError(SslUnsafeError::NoPeerCertificate);
    } else if (mode == SslUnsafeSocket::SslClientMode) {
        // Check the peer certificate itself. First try the subject's common name
        // (CN) as a wildcard, then try all alternate subject name DNS entries the
        // same way.

        // SslUnsafeSocket has a rather twisted logic: if verificationPeerName
        ...
        if (!SslUnsafeSocketPrivate::isMatchingHostname(dtlsConfiguration.peerCertificate, name))
            errors << SslUnsafeError(SslUnsafeError::HostNameMismatch, dtlsConfiguration.peerCertificate);
    }

    // Translate errors from the error list into SslUnsafeErrors
    errors.reserve(errors.size() + opensslErrors.size());
    for (const auto &error : const_cast<QVector<SslUnsafeErrorEntry> &>(opensslErrors)) { // qAsConst(opensslErrors)
        errors << _q_OpenSSL_to_SslUnsafeError(error.code,
...
void SslUnsafeDtlsPrivateOpenSSL::storePeerCertificates()
{
    Q_ASSERT(dtls.tlsConnection.data());
    ...
    dtlsConfiguration.peerCertificate = SslUnsafeCertificatePrivate::SslUnsafeCertificate_from_X509(x509);
    q_X509_free(x509);
    if (dtlsConfiguration.peerCertificateChain.isEmpty()) {
        auto stack = q_SSL_get_peer_cert_chain(dtls.tlsConnection.data());
        dtlsConfiguration.peerCertificateChain = SslUnsafeSocketBackendPrivate::STACKOFX509_to_SslUnsafeCertificates(stack);
        if (!dtlsConfiguration.peerCertificate.isNull() && mode == SslUnsafeSocket::SslServerMode)
            dtlsConfiguration.peerCertificateChain.prepend(dtlsConfiguration.peerCertificate);
    }
}

bool SslUnsafeDtlsPrivateOpenSSL::tlsErrorsWereIgnored() const
{
    // check whether the errors we got are all in the list of expected errors
    // (applies only if the method SslUnsafeDtlsConnection::ignoreTlsErrors(const
    // QVector<SslUnsafeError> &errors) was called)
    for (const SslUnsafeError &error : tlsErrors) {
...
void SslUnsafeDtlsPrivateOpenSSL::fetchNegotiatedParameters()
{
    Q_ASSERT(dtls.tlsConnection.data());

    const SSL_CIPHER *cipher = q_SSL_get_current_cipher(dtls.tlsConnection.data());
    sessionCipher = cipher ? SslUnsafeSocketBackendPrivate::SslUnsafeCipher_from_SSL_CIPHER(cipher)
                           : SslUnsafeCipher();
    ...
    case DTLS1_VERSION:
        sessionProtocol = SslUnsafe::DtlsV1_0;
        break;
    case DTLS1_2_VERSION:
        sessionProtocol = SslUnsafe::DtlsV1_2;
        break;
    default:
...
void SslUnsafeDtlsPrivateOpenSSL::reportTimeout()
{
    Q_Q(SslUnsafeDtls);
...
void SslUnsafeDtlsPrivateOpenSSL::resetDtls()
{
    dtls.reset();
    connectionEncrypted = false;
    tlsErrors.clear();
