...
QString SslUnsafeEllipticCurve::shortName() const
{
    QString result;
...
QString SslUnsafeEllipticCurve::longName() const
{
    QString result;
#ifndef OPENSSL_NO_EC
    if (id != 0)
...
SslUnsafeEllipticCurve SslUnsafeEllipticCurve::fromShortName(const QString &name)
{
    if (name.isEmpty())
        return SslUnsafeEllipticCurve();

    SslUnsafeSocketPrivate::ensureInitialized();

    SslUnsafeEllipticCurve result;
    ...
    if (nid == 0 && SslUnsafeSocket::sslLibraryVersionNumber() >= 0x10002000L)
...
SslUnsafeEllipticCurve SslUnsafeEllipticCurve::fromLongName(const QString &name)
{
    if (name.isEmpty())
        return SslUnsafeEllipticCurve();
...
bool SslUnsafeEllipticCurve::isTlsNamedCurve() const Q_DECL_NOTHROW
{
