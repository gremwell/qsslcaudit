...
void SslUnsafeContext::initSslContext(SslUnsafeContext *sslContext, SslUnsafeSocket::SslMode mode, const SslUnsafeConfiguration &configuration, bool allowRootCertOnDemandLoading)
{
    ...
    bool isDtls = false;
init_context:
    if (sslContext->sslConfiguration.protocol() == SslUnsafe::SslV2) {
    ...
    case SslUnsafe::SslV2:
        ...
        Q_UNREACHABLE();
        break;
    case SslUnsafe::UnknownProtocol:
        break;
    }

    if (minVersion != anyVersion
        && !q_SSL_CTX_set_min_proto_version(sslContext->ctx, minVersion)) {
        sslContext->errorStr = SslUnsafeSocket::tr("Error while setting the minimal protocol version");
        sslContext->errorCode = SslUnsafeError::UnspecifiedError;
        return;
    }
    ...
    if (maxVersion != anyVersion
        ...
        sslContext->errorStr = SslUnsafeSocket::tr("Error while setting the maximum protocol version");
        sslContext->errorCode = SslUnsafeError::UnspecifiedError;
        return;
    }
    ...
    auto filterCiphers = [](const QList<SslUnsafeCipher> &ciphers, bool selectTls13)
    {
        QByteArray cipherString;
        bool first = true;

        for (const SslUnsafeCipher &cipher : qAsConst(ciphers)) {
            const bool isTls13Cipher = cipher.protocol() == SslUnsafe::TlsV1_3 || cipher.protocol() == SslUnsafe::TlsV1_3OrLater;
    ...
    if (!preTls13Ciphers.size() && !tls13Ciphers.size()) {
        sslContext->errorStr = SslUnsafeSocket::tr("Invalid or empty cipher list (%1)").arg(QStringLiteral(""));
        sslContext->errorCode = SslUnsafeError::UnspecifiedError;
        return;
    }
    ...
    if (!sslContext->sslConfiguration.localCertificate().isNull()) {
        ...
        if (configuration.d->privateKey.algorithm() == SslUnsafe::Opaque)
            sslContext->pkey = nullptr; // Don't free the private key, it belongs to SslUnsafeKey
    ...
    if (!qcurves.isEmpty()) {
#ifdef OPENSSL_NO_EC
        sslContext->errorStr = msgErrorSettingEllipticCurves(SslUnsafeSocket::tr("OpenSSL version with disabled elliptic curves"));
        sslContext->errorCode = SslUnsafeError::UnspecifiedError;
        return;
#else
