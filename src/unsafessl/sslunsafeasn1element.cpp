...
SslUnsafeAsn1Element::SslUnsafeAsn1Element(quint8 type, const QByteArray &value)
    : mType(type)
    , mValue(value)
{
}

bool SslUnsafeAsn1Element::read(QDataStream &stream)
{
    // type
    quint8 tmpType;
    stream >> tmpType;
...
bool SslUnsafeAsn1Element::read(const QByteArray &data)
{
    QDataStream stream(data);
    return read(stream);
}

void SslUnsafeAsn1Element::write(QDataStream &stream) const
{
    // type
    stream << mType;
...
SslUnsafeAsn1Element SslUnsafeAsn1Element::fromBool(bool val)
{
    return SslUnsafeAsn1Element(SslUnsafeAsn1Element::BooleanType,
        QByteArray(1, val ? 0xff : 0x00));
}

SslUnsafeAsn1Element SslUnsafeAsn1Element::fromInteger(unsigned int val)
{
    SslUnsafeAsn1Element elem(SslUnsafeAsn1Element::IntegerType);
    while (val > 127) {
...
SslUnsafeAsn1Element SslUnsafeAsn1Element::fromVector(const QVector<SslUnsafeAsn1Element> &items)
{
    SslUnsafeAsn1Element seq;
    seq.mType = SequenceType;
    QDataStream stream(&seq.mValue, QIODevice::WriteOnly);
    for (QVector<SslUnsafeAsn1Element>::const_iterator it = items.cbegin(), end = items.cend(); it != end; ++it)
        it->write(stream);
    return seq;
}

SslUnsafeAsn1Element SslUnsafeAsn1Element::fromObjectId(const QByteArray &id)
{
...
bool SslUnsafeAsn1Element::toBool(bool *ok) const
{
...
QDateTime SslUnsafeAsn1Element::toDateTime() const
{
...
QMultiMap<QByteArray, QString> SslUnsafeAsn1Element::toInfo() const
{
    QMultiMap<QByteArray, QString> info;
    SslUnsafeAsn1Element elem;
    QDataStream issuerStream(mValue);
    while (elem.read(issuerStream) && elem.mType == SslUnsafeAsn1Element::SetType) {
        SslUnsafeAsn1Element issuerElem;
        QDataStream setStream(elem.mValue);
        if (issuerElem.read(setStream) && issuerElem.mType == SslUnsafeAsn1Element::SequenceType) {
            QVector<SslUnsafeAsn1Element> elems = issuerElem.toVector();
            if (elems.size() == 2) {
...
qint64 SslUnsafeAsn1Element::toInteger(bool *ok) const
{
    if (mType != SslUnsafeAsn1Element::IntegerType || mValue.isEmpty()) {
        if (ok)
            *ok = false;
        return 0;
    }
...
QVector<SslUnsafeAsn1Element> SslUnsafeAsn1Element::toVector() const
{
    QVector<SslUnsafeAsn1Element> items;
...
QByteArray SslUnsafeAsn1Element::toObjectId() const
{
    QByteArray key;
...
QByteArray SslUnsafeAsn1Element::toObjectName() const
{
    QByteArray key = toObjectId();
    return oidNameMap->value(key, key);
}

QString SslUnsafeAsn1Element::toString() const
{
