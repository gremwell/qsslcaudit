...
static QStringList findAllLibs(QLatin1String filter)
{
    ...
    for (const QString &path : paths) {
        ...
        for (const QString &entry : const_cast<const QStringList&>(entryList)) //qAsConst(entryList))
...
#ifdef UNSAFE
        libssl->setFileNameAndVersion(QLatin1String("unsafessl"), fallbackSoname);
        libcrypto->setFileNameAndVersion(QLatin1String("unsafecrypto"), fallbackSoname);
#else
