 ...
 */
bool CertificateBuilder::addAuthorityKeyIdentifier(const XSslCertificate &qcacert)
{
...
/*!
  ...
  key.
 */
XSslCertificate CertificateBuilder::signedCertificate(const XSslKey &qkey)
{
    gnutls_x509_privkey_t key = qsslkey_to_key(qkey, &d->errnumber);
    if (GNUTLS_E_SUCCESS != d->errnumber) {
        gnutls_x509_privkey_deinit(key);
        return XSslCertificate();
    };
...
/*!
  ...
  CA key.
 */
XSslCertificate CertificateBuilder::signedCertificate(const XSslCertificate &qcacert, const XSslKey &qcakey)
{
    //
    // Extract the CA key
    //
